...
# Note: Container Apps Contributor role and authentication are configured via Bicep during deployment
# Note: AI Foundry Project Application ID is retrieved in preprovision hook before Bicep deployment


# Get required values from azd environment
$containerAppPrincipalId = (azd env get-values | Select-String -Pattern '^SERVICE_API_IDENTITY_PRINCIPAL_ID=' | ForEach-Object { $_.Line.Split('=')[1].Trim() }) -replace '^"|"$', ''
$aiFoundryResourceId = (azd env get-values | Select-String -Pattern '^AI_FOUNDRY_RESOURCE_ID=' | ForEach-Object { $_.Line.Split('=')[1].Trim() }) -replace '^"|"$', ''
$aiFoundryProjectResourceId = (azd env get-values | Select-String -Pattern '^AI_FOUNDRY_PROJECT_RESOURCE_ID=' | ForEach-Object { $_.Line.Split('=')[1].Trim() }) -replace '^"|"$', ''

if (-not $aiFoundryResourceId) {
    Write-Error "AI_FOUNDRY_RESOURCE_ID must be set in azd environment"
    exit 1
}

if (-not $aiFoundryProjectResourceId) {
    Write-Error "AI_FOUNDRY_PROJECT_RESOURCE_ID must be set in azd environment"
    exit 1
}

if (-not $containerAppPrincipalId) {
    Write-Error "Could not find container app principal ID in azd environment (SERVICE_API_IDENTITY_PRINCIPAL_ID)"
    exit 1
}
...
$aiFoundryAccount = az cognitiveservices account show --name $projectAiFoundryName --resource-group $projectResourceGroup --query location -o tsv 2>$null
...
# Validate and assign Azure AI User role if needed
Write-Host "`n======================================"
Write-Host "Assigning Azure AI Access Permissions"
Write-Host "======================================"

$roleDefinitionId = "53ca6127-db72-4b80-b1b0-d745d6d5456d" # Azure AI User
$aiFoundryAccountScope = $aiFoundryResourceId

Write-Host "Assigning 'Azure AI User' role to Container App identity..."
Write-Host "Principal ID: $containerAppPrincipalId"
Write-Host "Scope: $aiFoundryAccountScope"
Write-Host ""

$assignmentJson = az role assignment create --assignee $containerAppPrincipalId --role $roleDefinitionId --scope $aiFoundryAccountScope 2>&1

if ($LASTEXITCODE -eq 0) {
    Write-Host "✓ Successfully assigned 'Azure AI User' role"
    $assignment = $assignmentJson | ConvertFrom-Json
    Write-Host "  Assignment ID: $($assignment.name)"
    Write-Host "  Created: $($assignment.createdOn)"
    Write-Host ""
    Write-Host "⏳ Waiting 30 seconds for RBAC propagation..."
    Start-Sleep -Seconds 30
    
    # Validate the assignment exists
    Write-Host "Validating role assignment..."
    $validationJson = az role assignment list --assignee $containerAppPrincipalId --scope $aiFoundryAccountScope --role $roleDefinitionId --query "[0]" -o json 2>$null
    
    if ($validationJson -and $validationJson.Trim() -ne "null" -and $validationJson.Trim() -ne "") {
        $validatedAssignment = $validationJson | ConvertFrom-Json
        Write-Host "✓ Role assignment validated successfully"
        Write-Host "  Role: $($validatedAssignment.roleDefinitionName)"
    } else {
        Write-Warning "Could not validate role assignment. It may still be propagating."
    }
} else {
    Write-Error "Failed to assign 'Azure AI User' role."
    Write-Error "Error: $assignmentJson"
    Write-Error ""
    Write-Error "This requires Owner or User Access Administrator role on the AI Foundry Account."
    Write-Error "Manual command:"
    Write-Error "az role assignment create \"
    Write-Error "  --assignee $containerAppPrincipalId \"
    Write-Error "  --role `"53ca6127-db72-4b80-b1b0-d745d6d5456d`" \"
    Write-Error "  --scope `"$aiFoundryAccountScope`""
    exit 1
}

# Get deployment name from azd environment variables
$deploymentName = (azd env get-values | Select-String -Pattern '^DEPLOYMENT_NAME=' | ForEach-Object { $_.Line.Split('=')[1].Trim() }) -replace '^"|"$', ''

if (-not $deploymentName) {
    Write-Error "DEPLOYMENT_NAME must be set in azd environment"
    exit 1
}
...
if ($resourceId) {
    ...
    if ($resourceId -match '/subscriptions/([^/]+)/resourceGroups/([^/]+)/.*/containerApps/([^/]+)$') {
        ...
        try {
            ...
            $revisionsJson = az containerapp revision list --name $appName --resource-group $resourceGroup --subscription $subscription --query "[].{name:name, image:properties.template.containers[0].image, active:properties.active}" -o json 2>$null
                ...
                if (-not $helloWorldRevision) {
                    ...
                    if ($helloWorldRevision.image -notlike "*containerapps-helloworld*") {
                        ...
                        if ($helloWorldRevision.active -eq $false) {
                            ...
                            az containerapp revision deactivate --name $appName --resource-group $resourceGroup --subscription $subscription --revision $helloWorldRevision.name 2>&1 | Out-Null
    ...
    Write-Host "`n======================================"
    Write-Host "Verifying Container App Authentication Configuration"
    Write-Host "======================================"
    try {
        $authConfigJson = az rest --method GET --uri "https://management.azure.com$resourceId/authConfigs/current?api-version=2024-03-01" 2>$null
        if ($authConfigJson) {
            ...
            if ($authConfig.properties.identityProviders.azureActiveDirectory) {
                ...
                Write-Host "`nAzure Active Directory Configuration:"
                Write-Host "  Enabled: $($aadConfig.enabled)"
                Write-Host "  Client ID: $($aadConfig.registration.clientId)"
                Write-Host "  OpenID Issuer: $($aadConfig.registration.openIdIssuer)"
                Write-Host "  Allowed Audiences: $($aadConfig.validation.allowedAudiences -join ', ')"
                Write-Host "  Allowed Applications: $($aadConfig.validation.defaultAuthorizationPolicy.allowedApplications -join ', ')"
            } else {
                Write-Warning "Azure Active Directory identity provider is not configured"
            }
        } else {
            Write-Warning "No authentication configuration found for Container App"
        }
    } catch {
        Write-Warning "Failed to retrieve authentication configuration: $($_.Exception.Message)"
    }
    
    # Get the Container App endpoint (FQDN) for testing
    Write-Host "Retrieving Container App endpoint..."
    $containerAppJson = az resource show --ids $resourceId --query properties.configuration.ingress.fqdn -o json 2>$null
    if ($containerAppJson) {
    ...
    $projectJson = az resource show --ids $aiFoundryProjectResourceId 2>$null
    ...
    if ($token) {
        ...
        $latestRevisionName = ''
        try {
            $latestRevisionName = az containerapp show --ids $resourceId --query properties.latestRevisionName -o tsv 2>$null
        ...
        # Check for AI_PROJECT_ENDPOINT environment variable
        $aiProjectEndpoint = (azd env get-values | Select-String -Pattern '^AI_PROJECT_ENDPOINT=' | ForEach-Object { $_.Line.Split('=')[1].Trim() }) -replace '^"|"$', ''
        ...
        if ($aiProjectEndpoint) {
            # Use the provided AI_PROJECT_ENDPOINT
            $uri = "$aiProjectEndpoint$apiPath"
            Write-Host "Using AI_PROJECT_ENDPOINT: $aiProjectEndpoint"
        } else {
            # Use regional API endpoint based on AI Foundry location
